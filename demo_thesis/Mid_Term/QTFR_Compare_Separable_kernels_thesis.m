function Compare_Separable_kernels_thesis
...
% 计算普通 WVD
wv=tlkern(s,N);
figure('Name','WVD');tfsapl(abs(s),wv,'Grayscale','on');
% 计算时间方向采用hann窗的滤波后信号
tfd=tlkern(s,N,1,'hann',15);%LI, time Hann 15
figure('Name','WVD');tfsapl(real(s),tfd,'Grayscale','on'); 
...
figure('Name','WVD, lag Hamm 47');tfsapl(real(s),tfd,'Grayscale','on'); 

% LI, time Hann 11
tfd=tlkern(s,N,1,'hann',11);
figure('Name','LI, time Hann 11');tfsapl(real(s),tfd,'Grayscale','on'); 

% Separable, time hann 11, lag Hamm 47
tfd=tlkern(s,N,1,'hann',11,'hamm',47);
figure('Name','Separable, time hann 11, lag Hamm 47');tfsapl(real(s),tfd,'Grayscale','on'); 

% MB alpha = 0.2
tfd=tlkern(s,N,1,'delta',1,'1',1,'mb',0.2);
figure('Name','MB 0.2');tfsapl(real(s),tfd,'Grayscale','on');

% Spectrogram, rect 35
tfd=tlkern(s,N,1,'delta',1,'rect',35,'sg');
figure('Name','Spectrogram, rect 35'); tfsapl(real(s),tfd,'Grayscale','on');
...
figure('Name','WVD, lag Hamm 23'); tfsapl(real(s),tfd,'Grayscale','on');
...
figure('Name','Separable, time hann 11, lag Hamm 23'); tfsapl(real(s),tfd,'Grayscale','on');

% CW 1
tfd=tlkern(s,N,1,'delta',1,'1',1,'cw',1); %Exponential Distribution
figure('Name','CW 1'); tfsapl(real(s),tfd,'Grayscale','on');

% Spectrogram, rect 17
tfd=tlkern(s,N,1,'delta',1,'rect',17,'sg');  %Spectrogram
figure('Name','Spectrogram, rect 17'); tfsapl(real(s),tfd,'Grayscale','on');
