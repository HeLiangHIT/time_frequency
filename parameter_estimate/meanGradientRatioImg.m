function rImg = meanGradientRatioImg(beta0, beta1, beta2, beta1fix, beta2fix, winHalf)
% 根据公式9计算计算均率图像Mean ratio images
% 调用：rImg = meanGradientRatioImg(beta0, beta1, beta2, beta1fix, beta2fix);

if nargin < 6; winHalf = 8; end; %窗大小，自适应窗的方向了

[maxI,maxJ] = size(beta0);%maxI表示垂直坐标最大值，maxJ表示水平坐标最大值
method = 'mirror';
iniLen = 3;%【最好是根据TFD选择，和TFD曲线宽度近似比较好】
ep = 1e-2;%阈值，放置最大值太大
sigmoid = @(x) winHalf;%4*iniLen*(1./(1+exp(-0.2*x)) - 0.5) + 1;%幅度--窗口长度：自适应函数
% 该自适应函数后期修改为固定值winHalf，因为只需要自适应窗的方向不需要自适应窗的大小，以避免出现小幅度时的错误。
...
for k1 = 1:maxI
    for k2 = 1:maxJ
        ...
        %% 绘图查看正确与否
        %veci(k1,k2).^2 +vecj(k1,k2).^2
%         if veci(k1,k2).^2 +vecj(k1,k2).^2 > 5
%             figure(1);clf;subplot(121)
%             imagesc(beta0);axis xy; grid off;
%             itmp = zeros(size(beta0));itmp(h, v) =neibor;
%             subplot(122);imagesc(itmp); axis xy; grid off; %hold on;
...
%         end
        ...
        cImen1 = sum(cImg1(:))/(squre+eps);cImen2 = sum(cImg2(:))/(squre+eps);%子图像均值
        cImen3 = sum(cImg3(:))/(squre+eps);cImen4 = sum(cImg4(:))/(squre+eps);
...
safeGuard = winLen*0.3;%保险边缘，放置自适应窗太窄
method = 'mirror';
if abs(veci)>abs(vecj)%窗的高度设置
    winHei = abs(winLen*vecj/(veci+eps)) + 1;%取绝对值，保证窗长度>高度
else
    winHei = abs(winLen*veci/(vecj+eps)) + 1;
end
% 长度对应向量方向，高度垂直向量方向
