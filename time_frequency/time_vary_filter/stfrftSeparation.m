function [sigs,tfr,tfrv,ucs] = stfrftSeparation(x,ifs,halfLen,h)
%% 基于短时傅立叶变换时变滤波的信号分离，固定窗长度：[sigs,tfr,tfrv] = stfrftSeparation(x,ifs,halfLen,h)
% 输入：x：输入信号，是多个分量的叠加
%           ifs：各列是各个分量的瞬时频率，
...
for it=1:tLen,
    ...
    for is = 1:sigN
        ...
        ftfr = tvfIter(tfr(:,it,is), ucs(it,is), halfLen);%频域滤波，频域的矩形窗。适当放宽限制。
        %信号时域值恢复
