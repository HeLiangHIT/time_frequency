function [s,s_itfr,s_org,iflaw] = signalGenLxLS()
...
%% LFM信号：不同位置的两个LFM信号
t1 = 100;%LFM1起始点
[s1,if1]=fmlin(400,0.45,0.25,1);%线性调频信号，其0相位为第一个采样点
s_org{1} = [zeros(t1,1);s1;zeros(N-t1-length(s1),1)];
[s1,if1] = insert_signal(s1,N,t1,if1);

t2 = 10;%LFM2起始点
[s2,if2]=fmlin(400,0.25,0.45,100);%线性调频信号，其0相位为第100个采样点
s_org{2} = [zeros(t2,1);s2;zeros(N-t2-length(s2),1)];
[s2,if2]= insert_signal(s2,N,t2,if2);

%% SIN调频信号产生
t4=20;
[s4,if4]=fmsin(450,0.05,0.16,200,1,0.1,1);%(N,FNORMIN,FNORMAX,PERIOD,T0,FNORM0,PM1)
