function [tfr,t,f] = tfrstft_my(x,t,N,h,trace);
%TFRSTFT Short time Fourier transform.
%	[TFR,T,F]=tfrstft_my(X,T,N,H,TRACE) computes the short-time Fourier
%	transform of a discrete-time signal X.
%
%	X     : signal.
%	T     : time instant(s)          (default : 1:length(X)).
%	N     : number of frequency bins (default : length(X)).
%	H     : frequency smoothing window, H 【not being normalized so as to
