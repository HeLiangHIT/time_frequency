...
%% script 对比各种多方向核的性能 Multi directional Kernel：
% 可以显示信号IAF域滤波的计算过程。
% MDD 核是根据本文所产生的信号生成的一个核函数，因此可以用来最佳本次产生的信号！！！
% 由此可以推断FRFT由于相当于IAF域的一条直线，因此可以其对LFM信号具有最佳的时频分析聚集效果？？
% 这个原理如何用数学上解释呢！！！？？
...
[am1, I]=wvdClean(sig,N);% 计算AF域
% figure;tfsapl(sig,I,'Title','WVD', 'GrayScale','on');%WVD分布
...
subplot(236);imagesc(abs(am));axis xy; title('IAF*kenel');%等效于WVD卷积kernal
