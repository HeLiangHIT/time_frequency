...
s = awgn(s_org,0,'measured');%%%%%%%%% 5, 0, -5
tfr = quadtfd(s, 127, 1, 'emb', 0.1, 0.3, N)';%tfr = tfrADTFD(s,2,15,82)';
...
legend([hmcq,hfit],{'IF片段','拟合IF片段'}); set_gca_style([6,6]);grid off; ylim([0,60]);%方便放置图例
xlabel('时间/\mus');ylabel('频率/Mhz');
