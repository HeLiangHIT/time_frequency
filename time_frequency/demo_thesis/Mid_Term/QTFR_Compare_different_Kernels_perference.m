function QTFR_Compare_different_Kernels_perference
%% script 对比不同QTFR设计核下的TFR的性能
...
R0=sum(abs(WV(:))>0.1*max(abs(WV(:))))/N.^2
...
% Kurtosis1 = sum(abs(tfd1(:)).^4)/sum(abs(tfd1(:)).^2).^2
R1=sum(abs(tfd1(:))>0.1*max(abs(tfd1(:))))/N.^2
...
tfd1 = (fft(ifftshift(am,1), [], 1));
tfd1=  ifft(fftshift(tfd1,2), [], 2);
% Kurtosis2 = sum(abs(tfd1(:)).^4)/sum(abs(tfd1(:)).^2).^2
R2=sum(abs(tfd1(:))>0.1*max(abs(tfd1(:))))/N.^2
...
% Kurtosis3 = sum(abs(tfd1(:)).^4)/sum(abs(tfd1(:)).^2).^2
R3=sum(abs(tfd1(:))>0.1*max(abs(tfd1(:))))/N.^2
