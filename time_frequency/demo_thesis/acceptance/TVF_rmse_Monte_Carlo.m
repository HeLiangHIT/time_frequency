function rmse = TVF_rmse_Monte_Carlo(s_org,s_cpnt,sif,SNR,testN, LenStft, LenStfrft, LenOverlap)
...
sigN = size(s_cpnt,2);
LEN_SNR = length(SNR);
rmse = zeros(LEN_SNR,sigN,9);%两个算法对比
for k = 1:LEN_SNR
    ...
    for n = 1:testN
        ...
        [sh9,~,~] = synsqCwtSeparationAdv(s,sif,LenStfrft);
        ...
        for sign = 1:sigN
            ...
            rmse(k,sign,9)=rmse(k,sign,9) + sqrt(mean(abs(sh9(:,sign)-s_cpnt(:,sign)).^2));
        end
    end
